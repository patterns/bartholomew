...
test "mbedcrypto pkcs1 verify" {
    ...
    // is the signature expected by mbedcrypto another format?
    // (we have a base64 encoded text string)
    // guessing from the example rsa_verify.c:
    // fscanf(f, "%02X", (unsigned int*) &c) means
    // the .sig file has hexadecimal. The fscanf reads hex and
    // pushes that as "unsigned char" onto the array.
    // a) attempt decode the base64 and feed that result into verify
...
// print log text for debugging
fn log_error(prefix: []const u8, ret: c_int) void {
    if (ret == 0) return;

    var txt: [80]u8 = undefined;
    c.mbedtls_strerror(ret, &txt, 80);
    std.log.warn("DBG {s}, {s}", .{ prefix, txt });
}
