...
// modified from std.crypto.Certificate file
pub const snippet = struct {
    pub fn verifyRsa(
        ...
        switch (modulus.len) {
            inline 128, 256, 512 => |modulus_len| {
                const ps_len = modulus_len - (hash_der.len + hashed.len) - 3;
    ...
    fn encrypt(
        ...
        try pow_montgomery(&e, &m, &exponent, &modulus, allocator);
